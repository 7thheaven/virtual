...
#define xenbus_alloc_evtchn_id 6
...
bool useit[jpnum]={1,1,1,1,1,1,1};  //if true then register probe
...
void my_xen_l2_entry_update(pmd_t *ptr, pmd_t val)
{
    ...
    if(time>0) --time; else {signal=0;printk("Fortune: Done.\n");jprobe_return();}
    ...
    if(fault<0)    //fault less than 0 means just watch & change nothing
    {
        mmuupdateop.ptr = mmuupdateop.ptr;
        ptr->pmd = ptr->pmd;
        mmuupdateop.val = mmuupdateop.val;
        //just watch it, change nothing
        printk("Fortune: just watch.\n");
    }
    else if(fault==0)
    {
        getrando(64);
...
void my_xen_pgd_pin(unsigned long ptr)
{
    ...
    if(fault<0)
    {
        mmuextop.cmd = mmuextop.cmd;
        mmuextop.arg1.mfn = mmuextop.arg1.mfn;
        //just watch it, change nothing
        printk("Fortune: just watch.\n");
    }
    else if(fault==0)
...
asmlinkage long my_sys_ioctl(unsigned int fd, unsigned int cmd, unsigned long arg)
{
    ...
    //getdate();
    if(cmd==IOCTL_PRIVCMD_HYPERCALL)	//IOCTL_PRIVCMD_HYPERCALL
    {
        //++ioctlcount;
        ...
        printk("Fortune: hypercall op is %d.\n",orig_op);
        if(fault<0)
            jprobe_return();
        ...
        {
            orig_cmd=hypercall.arg[1];
            orig_arg=hypercall.arg[2];
            printk("cmd=%u\targ=0x%lx\n",orig_cmd,orig_arg);
            if(fault==0)
            {
                getrando(32);
                orig_cmd ^= (1 << rando);
                printk("Fortune: change orig_cmd to %u\n",orig_cmd);
            }
            if(fault==1)
            {
                getrando(32);
                orig_arg = 0;
                printk("Fortune: change arg to 0x%lx\n",orig_arg);
            }
            hypercall.arg[1]=orig_cmd;
            hypercall.arg[2]=orig_arg;
            if(copy_to_user(udata, &hypercall, sizeof(hypercall)))
            {
                printk("Fortune: B copy_to_user error\n");
                jprobe_return();
...
int my_xenbus_alloc_evtchn(struct xenbus_device *dev, int *port)
{
    
    //dev->otherend_id;
    printk("Fortune: xenbus_alloc_evtchn from %s domid %d\n",current->comm,dev->otherend_id);
    jprobe_return();
    return 0;
}
...
static int mvmhyper_init(void)
{
    ...
    if(useit[xenbus_alloc_evtchn_id])
    {
        jphyper[xenbus_alloc_evtchn_id].entry = (kprobe_opcode_t *) my_xenbus_alloc_evtchn;
        jphyper[xenbus_alloc_evtchn_id].kp.symbol_name="xenbus_alloc_evtchn";
    }
