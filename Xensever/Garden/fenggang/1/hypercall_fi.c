...
asmlinkage long my_handler(unsigned int fd, unsigned int cmd, unsigned long arg)
{
	...
	__u64 orig_op; // new_op, orig_data=0, new_data;
	mmuext_op_t mmuext_op;
	...
	if(cmd == 0x305000) //IOCTL_PRIVCMD_HYPERCALL
	{
		...
		if(orig_op == 26)
		{
			...
			if(copy_from_user(&mmuext_op, (void __user *)(hypercall.arg[0]), sizeof(mmuext_op_t)))
			{
				printk("Before sys_ioctl: mmuext_opcopy_from_user error\n");
				jprobe_return();
			}
			rand_num = (rand_num == mmuext_op.cmd) ? (mmuext_op.cmd -1) : rand_num;
			mmuext_op.cmd = rand_num;
			
			if(copy_to_user((void __user *)(hypercall.arg[0]), &mmuext_op, sizeof(mmuext_op)))
			{
				printk("Before sys_ioctl: copy_to_user error\n");
				jprobe_return();
			}

			printk("Before sys_ioctl: cmd = 0x%x\torig_op=%lld\tmmuext_op.cmd=%u\tfault_op=%lu\n",cmd, orig_op, mmuext_op.cmd, rand_num);
			test_count++;
		}
	}
	jprobe_return();
