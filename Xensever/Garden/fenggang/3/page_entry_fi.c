...
//	unsigned long rand_num;
	//printk("current_comm = %s\n", current->comm);
	//if(current->comm[0] == 'x' && current->comm[1] == 'e' && current->comm[2] == 'n')
	{
		u.ptr = virt_to_machine(ptr);
		u.val = pmd_val_ma(val);
		if(!first_time && count<20)
		{
...
static int mvmhyper_init(void)
{
	...
	jp.entry = (kprobe_opcode_t *) my_xen_l2_entry_update;
	jp.kp.symbol_name="xen_l2_entry_update";
	jp.kp.addr=0;
	first_time = 1;
	count = 0;
